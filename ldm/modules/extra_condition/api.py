...
def get_cond_sketch(opt, cond_image, cond_inp_type, cond_model=None, nigger = False, count=0):
    ...
    if nigger:
        # for white background, black sketch
        edge = 1-edge 
    ...
    for i, edge_image in enumerate(edge):
        edge_image = edge_image.squeeze().cpu().numpy()
        edge_image = (edge_image * 255).astype(np.uint8)
        cv2.imwrite(f'./outputs/test-sketch/image_{count}.jpg', edge_image)
